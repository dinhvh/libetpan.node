...
var Capabilities = require('../lib/libetpan').option.Capabilities;

test('empty response should throw error', function(t) {
  var emptyRes = '';
  var r = etpan.responseParse(emptyRes, etpan.PARSER_ENABLE_GREETING);
  t.equal(r.result, etpan.MAILIMAP_ERROR_PARSE);
  t.end();
});
...
test('greeting response without crlf', function(t) {
  ...
  var r = etpan.responseParse(greetingRes, etpan.PARSER_ENABLE_GREETING);
